DROP DATABASE reviews;

CREATE DATABASE reviews;

USE reviews;

CREATE TABLE review (
  `product` varchar(12) primary key,
  `page` int,
  `count` int
);

CREATE TABLE results (
      `review_id` int AUTO_INCREMENT primary key,
      `review_product` varchar(12),
      `rating` int,
      `summary` varchar(50),
      `recommend` Boolean,
      `response` varchar(250) default NULL,
      `body` varchar(250),
      `date` Date,
      `reviewer_name` varchar(20),
      `helpfulness` int,
      `reported` Boolean,
      INDEX rev_ind (review_product),
      FOREIGN KEY (review_product)
      REFERENCES review(product)
);
/*
      `photos` [{
          `id` int auto_increment,
          `url` varchar(150)
        },
      ] */
CREATE TABLE photos (
          `id` int auto_increment primary key,
          `url` varchar(150),
          `review_id` int,
          INDEX phot_ind (review_id),
          FOREIGN KEY (review_id)
          REFERENCES results(review_id)
);

CREATE TABLE reviewmeta (
  `product_id` varchar(12) primary key,
  `ratings` {
    `2` int,
    `3` int,
    `4` int,
  },
  `recommended` {
    `0` int
  },
  `characteristics` {
    `Size` {
      `id` int,
      `value` varchar(10)
    },
    `Width` {
      `id` int,
      `value` varchar(10)
    },
    `Comfort` {
      `id` int,
      `value` varchar(10)
    },
  }
);





/* META */
{
  "product_id": "2",
  "ratings": {
    2: 1,
    3: 1,
    4: 2,
    // ...
  },
  "recommended": {
    0: 5
    // ...
  },
  "characteristics": {
    "Size": {
      "id": 14,
      "value": "4.0000"
    },
    "Width": {
      "id": 15,
      "value": "3.5000"
    },
    "Comfort": {
      "id": 16,
      "value": "4.0000"
    },
    // ...
}
/* REVIEWS */
{
  "product": "2",
  "page": 0,
  "count": 5,
  "results": [
    {
      "review_id": 5,
      "rating": 3,
      "summary": "I'm enjoying wearing these shades",
      "recommend": false,
      "response": null,
      "body": "Comfortable and practical.",
      "date": "2019-04-14T00:00:00.000Z",
      "reviewer_name": "shortandsweeet",
      "helpfulness": 5,
      "photos": [{
          "id": 1,
          "url": "urlplaceholder/review_5_photo_number_1.jpg"
        },
        {
          "id": 2,
          "url": "urlplaceholder/review_5_photo_number_2.jpg"
        },
        // ...
      ]
    },
    {
      "review_id": 3,
      "rating": 4,
      "summary": "I am liking these glasses",
      "recommend": false,
      "response": "Glad you're enjoying the product!",
      "body": "They are very dark. But that's good because I'm in very sunny spots",
      "date": "2019-06-23T00:00:00.000Z",
      "reviewer_name": "bigbrotherbenjamin",
      "helpfulness": 5,
      "photos": [],
    },
    // ...
  ]
}